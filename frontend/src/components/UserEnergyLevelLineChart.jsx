...
class UserEnergyLevelLineChart extends Component {
  ...
  searchDataPoint = () => {
    const dps = this.state.dataPoints;
    ...
    if (this.mouseDown) {
      ...
      this.timerId = setTimeout(() => {
        if (this.selected !== null) {
          ...
          dataPoints[this.selected].y = this.yValue;
          this.setState({ dataPoints }, () => {
            chart.options.data[0].dataPoints = this.state.dataPoints;
            chart.render();
            this.props.onChange(this.state.dataPoints);  // Pass dataPoints to parent component
          });
        }
      }, 0);
    } else {
